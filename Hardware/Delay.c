...
uint8_t per_us;				//每1us定时器节拍
uint32_t per_ms;				//每1ms节拍，注意168MHz下值为168000，需要32位，移植自STM32F0，此处谨慎
...
{
	uint32_t ticks_old=SysTick->VAL;		//前一个计数值
	uint32_t ticks_new;					//后一个计数值
	uint16_t ticks_sum=0;				//已经经过的节拍
	uint16_t ticks_delta=us*per_us;		//需要经过的节拍
...
void Delay_Ms(uint16_t ms)				//毫秒延时
{
	uint32_t ticks_old=SysTick->VAL;		//前一个计数值
	uint32_t ticks_new;					//后一个计数值
	uint32_t ticks_sum=0;				//已经经过的节拍
	uint32_t ticks_delta=ms*per_ms;			//需要经过的节拍
...
void Delay_Us_2(uint32_t us)				//微秒延时，范围更大
{
	...
	uint32_t ticks_new;					//后一个计数值
	uint32_t ticks_sum=0;				//已经经过的节拍
	uint32_t ticks_delta=us*per_us;		//需要经过的节拍
